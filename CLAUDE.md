...
### 1. **Entry page Revamp** (MEDIUM PRIORITY)
**Problem**: There is no proper entry page for the user to load in on
**Current**: When user opens the app, it goes to the Welcome Screen (Welcome to Lifequest)
**Goal**: Recreate the Welcome Scene into a entry page for all users
**Tasks**:
- Simple entry page that will have a background (generated by MidJourney)
- Spot on the upper part of the screen for the Life Quest Logo (Needs to be imported)
- Two buttons on the lower part of the page: Signup with Google and Continue as Guest
- Button generation code already exists on the WelcomeScene.gd, Signup with Google needs to be attached to the exisitng Oauth (Last update broke it), then redirected to the Tutorial onboarding process (Pending on task 2, so can redirect to tavern for now)
- Continue as Guest kicks off Generate Guest account (should already be setup in code, then redirected the same way as the process above.)
